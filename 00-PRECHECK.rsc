...
:if ([:len [/interface bridge find where name="bridgeLocal"]] = 0) do={ :error "PRECHECK FAIL: bridgeLocal missing" }
:if ([:len [/ip address find where address="192.168.1.1/24" and interface="bridgeLocal"]] = 0) do={ :error "PRECHECK FAIL: LAN gateway 192.168.1.1/24 missing from bridgeLocal" }
...
:put "PRECHECK PASS: ether1 DHCP WAN + bridgeLocal LAN"
