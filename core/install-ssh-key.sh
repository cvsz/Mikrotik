...
Validate local SSH key material, derive and fingerprint its public half, then use
...
[[ -f "$PRIVATE_KEY" ]] || { echo "SSH key file not found: $PRIVATE_KEY" >&2; exit 2; }
...
echo 'Secret SSH key material remains local and is never transmitted.'
