...
# zOS manages only rules carrying the PoliceDBC: comment prefix. Existing
# unrelated rules in these named chains are preserved. An unrelated jump to a
# zOS chain is a conflict rather than an invitation to take ownership.
:if ([:len [/ip firewall filter find where chain="input" and jump-target="ZEAZ-PoliceDBC-INPUT" and comment="ZEAZ-PoliceDBC: INPUT POLICY"]] = 0) do={
    :if ([:len [/ip firewall filter find where chain="input" and jump-target="ZEAZ-PoliceDBC-INPUT"]] > 0) do={
        :error "input already jumps to ZEAZ-PoliceDBC-INPUT without zOS ownership"
    }
...
:if ([:len [/ip firewall filter find where chain="forward" and jump-target="ZEAZ-PoliceDBC-FORWARD" and comment="ZEAZ-PoliceDBC: FORWARD POLICY"]] = 0) do={
    :if ([:len [/ip firewall filter find where chain="forward" and jump-target="ZEAZ-PoliceDBC-FORWARD"]] > 0) do={
        :error "forward already jumps to ZEAZ-PoliceDBC-FORWARD without zOS ownership"
    }
...
:if ([:len [/ip firewall nat find where chain="srcnat" and jump-target="ZEAZ-PoliceDBC-SRCNAT" and comment="ZEAZ-PoliceDBC: SRCNAT POLICY"]] = 0) do={
    :if ([:len [/ip firewall nat find where chain="srcnat" and jump-target="ZEAZ-PoliceDBC-SRCNAT"]] > 0) do={
        :error "srcnat already jumps to ZEAZ-PoliceDBC-SRCNAT without zOS ownership"
    }
