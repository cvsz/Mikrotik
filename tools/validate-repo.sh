...
required=(
  ...
  AGENTS.md README.md ENVIRONMENTS.md Makefile
  config/topology.env.example
  tools/omega-router.sh tools/deploy-phases.sh tools/core-network-repair.sh
)
...
grep -q 'OMEGA_ALLOW_LIVE_APPLY=0' config/topology.env.example || err 'live apply must default disabled'
