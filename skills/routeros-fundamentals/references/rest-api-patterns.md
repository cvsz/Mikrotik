...
status alone — a 200 from a resource endpoint is not proof of effect either.
...
// With a placeholder password — never commit a real credential
const auth = {
  headers: { Authorization: `Basic ${btoa("admin:REPLACE_ME")}` },
};
```
...
**Dangerous paths to skip:** `where`, `do`, `else`, `rule`, `command`, `on-error` — these can crash the REST server when inspected on affected versions/configurations.
...
Vendored for zOS from proposed upstream `tikoci/routeros-skills` PR #20 at commit
`74bd78b39c84507b9126e8526583d578bb0598d0`, with the example credential changed to an
explicit placeholder for repository safety.
