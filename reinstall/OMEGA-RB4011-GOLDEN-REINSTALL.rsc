...
:if ([:len [/interface bridge find where name="bridgeLocal"]] = 0) do={ /interface bridge add name=bridgeLocal protocol-mode=rstp comment="OMEGA LAN" }
...
:foreach ifn in={"ether2";"ether3";"ether4";"ether5";"ether6";"ether7";"ether8";"ether9";"ether10";"sfp-sfpplus1"} do={
    :if ([:len [/interface find where name=$ifn]] > 0 && [:len [/interface bridge port find where interface=$ifn]] = 0) do={ /interface bridge port add bridge=bridgeLocal interface=$ifn }
}
...
:if ([:len [/interface list member find where list="LAN" and interface="bridgeLocal"]] = 0) do={ /interface list member add list=LAN interface=bridgeLocal }
